...
用于审查 PPTX 与 HTML 转换或重建后的版式、动画和导出保真度。

## 协议快照

- 原始来源：`skills/pptx-html-fidelity-audit/SKILL.md`
- Skill 名称：`pptx-html-fidelity-audit`
- 触发词：`pptx fidelity`, `pptx audit`, `ppt 跑掉`, `字型不對`, `footer overlap`, `verify pptx`, `html to pptx`
- `mode`: `utility`
- `scenario`: `engineering`
