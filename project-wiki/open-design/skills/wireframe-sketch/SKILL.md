...
用于生成低保真线框、草图化界面和布局探索。

## 协议快照

- 原始来源：`skills/wireframe-sketch/SKILL.md`
- Skill 名称：`wireframe-sketch`
- 触发词：`wireframe`, `sketch wireframe`, `lo-fi mockup`, `hand drawn`, `whiteboard sketch`, `low fidelity`, `手绘原型`, `草图`, `线框图`
- `mode`: `prototype`
- `scenario`: `design`
- `platform`: `desktop`
