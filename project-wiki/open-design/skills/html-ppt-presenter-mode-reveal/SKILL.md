...
# 演讲者模式 Reveal HTML PPT 技能

> 这是 DeepWiki 生成的中文阅读镜像。当前环境的本机翻译模型不可用，因此这里保留完整原始 `SKILL.md` 正文，并在前面增加中文导读、协议快照和章节索引。复用该技能时，请以“原始正文”中的命令、路径、代码块、检查清单和输出契约为准。

## 中文摘要

用于生成带 presenter mode、reveal 节奏或演讲辅助结构的网页 PPT。

## 协议快照

- 原始来源：`skills/html-ppt-presenter-mode-reveal/SKILL.md`
- Skill 名称：`html-ppt-presenter-mode`
- 触发词：`presenter mode`, `演讲者模式`, `逐字稿`, `speaker notes`, `提词器`, `presenter view`, `演讲`
- `mode`: `deck`
- `scenario`: `engineering`
- `featured`: `26`
