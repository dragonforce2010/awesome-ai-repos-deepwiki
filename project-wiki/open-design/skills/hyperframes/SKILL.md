...
用于用 HTML、CSS、GSAP 和 HyperFrames 生成视频合成、字幕、转场和渲染流程。

## 协议快照

- 原始来源：`skills/hyperframes/SKILL.md`
- Skill 名称：`hyperframes`
- 触发词：`hyperframes`, `html video`, `video composition`, `interactive video`, `captions`, `tts video`, `kinetic typography`
- `mode`: `video`
- `surface`: `video`
- `scenario`: `video`
