...
description: 当你要为新站点编写 OpenCLI adapter，或给已有站点增加新命令时使用。它从首次侦察、字段解码、adapter 编码到 verify 给出端到端流程，替代 opencli-oneshot / opencli-explorer。临时浏览器操作请看 opencli-browser；opencli 顶层导航请看 opencli-usage。
